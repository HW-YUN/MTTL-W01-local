...
| `1.0.110` | 이후 투오 님이 공개한 Standalone(쉬운 설치) 구성용 커스텀 펌웨어. 중간 서버 없이 TCP 10086 backend 에 직접 연결하고 event push 를 사용하는 등 동작 구조가 다르며, 탱즈님의 순정 펌웨어 TCP 10086 polling 접근을 계승·개선한 흐름입니다. `1.0.106` 계보의 후속 발전형으로, `1.0.106` 과는 다른 버전이지만 무관한 별개 계열은 아닙니다. **본 프로젝트는 `1.0.110` 을 사용하지 않습니다** |
